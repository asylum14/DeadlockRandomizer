...
for i, wset in enumerate(weaponSets):
    ...
    for j, ability in enumerate(lAbilities):
        ...
        if ability =="citadel_ability_lash =":
            newability = newability.replace("citadel_ability_lash =","citadel_ability_sky_grapple =")
        if ability =="citadel_ability_lash_ultimate =":
              newability = newability.replace("citadel_ability_lash_ultimate =","citadel_ability_cloud_pull =")
        if ability =="citadel_ability_lash_down_strike =":
            newability = newability.replace("citadel_ability_lash_down_strike =","citadel_ability_high_slam =")
...
for i, char in enumerate(chars):
    ...
    if char =="hero_lash":
        output = output.replace("citadel_ability_lash_down_strike","citadel_ability_high_slam")
        output = output.replace("citadel_ability_lash_ultimate","citadel_ability_cloud_pull")
        output = output.replace("citadel_ability_lash","citadel_ability_sky_grapple")
    if i == len(chars)-1:
        break
