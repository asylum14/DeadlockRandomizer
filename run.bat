...
echo [[step]] > "%DeadPacker_directory%/Truerandom.toml"
echo [step.compile] >> "%DeadPacker_directory%/Truerandom.toml"
echo resource_compiler_path = '%CSDK_directory%/game/bin/win64/resourcecompiler.exe' >> "%DeadPacker_directory%/Truerandom.toml"
echo addon_content_directory = '%CSDK_directory%/content/citadel_addons/TrueRandom' >> "%DeadPacker_directory%/Truerandom.toml"

echo [[step]] >> "%DeadPacker_directory%/Truerandom.toml"
echo [step.pack] >> "%DeadPacker_directory%/Truerandom.toml"
echo input_directory = '%CSDK_directory%/game/citadel_addons/TrueRandom' >> "%DeadPacker_directory%/Truerandom.toml"
echo output_path = '%deadlockAddondir%/pak01_dir.vpk' >> "%DeadPacker_directory%/Truerandom.toml"
echo exclude = ['cache_*.soc', 'tools_thumbnail_cache.bin'] >> "%DeadPacker_directory%/Truerandom.toml"

echo %DeadPacker_directory%
start %DeadPacker_directory%\Deadpacker.exe %DeadPacker_directory%\TrueRandom.toml /s
timeout /t 5
